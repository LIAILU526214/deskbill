...
import com.deskbill.domain.Ledger;
...
public abstract class AbstractLedgerMngDialog extends JDialog {
	private static final long serialVersionUID = 4321618862747788709L;
	...
	private void addComponent() {
		...
		parentBox.setModel(new DefaultComboBoxModel<Object>(new Object[] {"收入/支出", "收入", "支出"}));
		...
		sortBox.setModel(new DefaultComboBoxModel<Object>(new Object[]{"全部分类"}));
	 ...
	 * 显示账务表格
	 */
	protected void setTableModel(List<BillCostDetail> ledgerList) {
		String[] colNames = new String[] {"ID", "收/支", "分类", "金额", "账户", "创建时间", "说明"};
		String[] propNames = new String[] {"id", "type", "costType", "money", "account", "costTime", "ddesc"};
		...
		try {
			ledgerDataTable.setModel(new ListTableModel<BillCostDetail>(ledgerList, colNames, propNames));
	...
	private void addListener() {
		...
		editBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				int selectedRow = ledgerDataTable.getSelectedRow();
				if(selectedRow == -1) {
					editBtn.setEnabled(false);
					delBtn.setEnabled(false);
					return;
				}
				String columnName = (String)ledgerDataTable.getModel().getValueAt(selectedRow, 0);
				if (columnName == null || "".equals(columnName)) {
					editBtn.setEnabled(false);
					delBtn.setEnabled(false);
					return;
				}
				editLedger(Long.parseLong(columnName));
			}
		});
		/*
		 * 删除账务按钮事件监听器
		...
		delBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				deleteLedger(Long.parseLong(columnName));
			}
		});
		/*
		 * 选择分类，父分类下拉菜单事件
		...
		ledgerDataTable.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if (ledgerDataTable.getSelectedRow() != -1) {
	 ...
	 * 添加账务
	 */
	public abstract void editLedger(Long id);
	
	/**
	 * 删除账务
	 */
	public abstract void deleteLedger(Long id);
